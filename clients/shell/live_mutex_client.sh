...
#
# Speaks the newline-delimited JSON wire protocol specified in ../PROTOCOL.md.
# The only dependency is bash itself (3.2+, with net redirections — the default
# on macOS and Linux): the transport is bash's built-in /dev/tcp, so there is no
# nc / python / jq dependency. zsh/sh users run the scripts directly; the shebang
# selects bash. Source this file and call the lmx_* functions — see smoke.sh.
...
# replies when the client is strictly older than it supports, so this is
# forward-safe.
: "${LMX_VERSION:=0.2.25}"
: "${LMX_TIMEOUT:=30}"
...
lmx_json_array() { local out="" k; for k in "$@"; do out="$out,\"$k\""; done; printf '[%s]' "${out:1}"; }
...
lmx_connect() {
  ...
  _lmx_send "$(printf '{"type":"%s","value":"%s"}' "$LMX_REQ_VERSION" "$LMX_VERSION")"
}
...
lmx_release() {
  ...
  _lmx_send "$(printf '{"type":"%s","uuid":"%s","_uuid":"%s","key":"%s"%s}' \
    "$LMX_REQ_UNLOCK" "$uuid" "$lock" "$key" "$forcef")"
...
lmx_release_many() {
  ...
  _lmx_send "$(printf '{"type":"%s","uuid":"%s","lockUuid":"%s"}' "$LMX_REQ_RELEASE_MANY" "$uuid" "$lock")"
